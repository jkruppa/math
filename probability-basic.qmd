...
: Wir schreiben $Pr(A \mid B)$, wenn wir die Wahrscheinlichkeit für das Ereignis $A$ bestimmen wolln unter der Bedingung das $B$ schon eingetreten ist. 
...
Die bedingte Wahrscheinlichkeit $Pr(A\mid B)$ können wir uns auch als Fläche*anteil* vorstellen. Im Folgenden haben wir die Ereignisse $A$ und $B$, die sich überschneiden. In der lila Fläche finden die Ereignisse $A$ und $B$ statt. Wir schreiben hier $A \cap B$. 

![](images/schnitt_2.png){fig-align="center" width="50%"}

Wenn $B$ gilt, wie wahrscheinlich sind wir dann in der lila Fläche, also gilt $A$ und $B$? Das können wir dann einfach über den Anteil der Fläche $A\cap B$ an der Fläche $B$ berechnen. Das wiederum ist nichts anderes als die bedingte Wahrscheinlichkeit, dass $A$ gilt, wenn $B$ wahr ist.

$$
Pr(A\mid B) = \cfrac{Pr(A \cap B)}{P(B)}
$$
...
Für zwei Ereignisse $A$ und $B$ mit der Eintrittswahrscheinlichkeit $Pr(B) > 0$ lässt sich die Wahrscheinlichkeit von $A$ unter der Bedingung, dass $B$ eingetreten ist, durch die Wahrscheinlichkeit von $B$ unter der Bedingung, dass $A$ eingetreten ist, errechnen:

$$
Pr(B\mid A) = \cfrac{Pr(A\mid B) \cdot Pr(B)} {Pr(A)}
$$
